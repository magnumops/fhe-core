...
def test_full_chain():
    ...
    try:
        ...
        print("ERROR: Binaries missing."); return

    modulus = config[0]
    sim = logos_emu.LogosContext()
    sim.reset_state()
    
    SRC_ADDR, RES_ADDR = 0x1000, 0x8000
    
    # 1. Write Input
    print(f"[Host] Writing {len(vec_a)} words...")
    for i, val in enumerate(vec_a): logos_emu.write_ram(SRC_ADDR + i*8, val)
    
    # 2. Exec: LOAD -> SQUARE
    print("[Host] Sending LOAD...")
    sim.push_command(2, 0, SRC_ADDR, 0)
    for _ in range(5000): sim.tick()
    
    print("[Host] Sending CALC (SQUARE)...")
