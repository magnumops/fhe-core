...
*   **Контекст:** Постоянная запись VCD создает файлы размером в гигабайты, замедляя CI.
*   **Решение:** Внедрить API `start_trace/stop_trace` и Python-менеджер контекста `TraceGuard`.
*   **Результат:** Трейсы пишутся только при падении теста (`ERROR_*.vcd`).

## ADR-007: Persistent Hardware Performance Counters
*   **Контекст:** Нам нужны метрики производительности (такты, операции), которые переживают мягкий сброс (`rst`) между этапами вычислений.
*   **Решение:**
    1.  Счетчики вынесены в отдельный `always` блок в Verilog, игнорирующий `rst`.
    2.  Чтение через выделенный опкод `OPC_READ_PERF` и DMA.
    3.  DPI-передача использует безопасную сборку `uint64` из двух `uint32`.
*   **Статус:** Implemented.
