...
void init_seal_env() {
    ...
    poly_modulus = parms.coeff_modulus()[0].value();
}
...
extern "C" {
    ...
    long long read_ram(long long addr) {
        ...
        shadow_mem[load_ptr % 4096] = (uint64_t)val; // Spy on data
        load_ptr++;
        return val;
    }
    ...
    // Init/Cleanup
    void init_ram() { global_ram.clear(); load_ptr = 0; init_seal_env(); }
    ...
    void dpi_exec_alu(int opcode, int slot, int count) {
        ...
        if (opcode == 7) { // MULT (Square Test)
            ...
            for(int i=0; i<4096; i++) {
                ...
                // Write result to Result RAM area (0x8000)
